 ...
 * @author fptshop
 */
@WebServlet(name="InsertStaffServlet", urlPatterns={"/manager/insertStaff"})
public class InsertStaffServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //int imageId = Integer.parseInt(request.getParameter("imageId"));
            ...
            Role role = roleDAO.selectById(Integer.parseInt(request.getParameter("roleId")));
            //int roleId = 5;

        // Tạo mật khẩu ngẫu nhiên
            String password = generateRandomPassword(3);

            Staff newStaff = new Staff(fullName, password, phoneNumber, cccd, mail, dob, sex, status,image, role);
...
//        request.getRequestDispatcher("manageResident").forward(request, response);
    ...
    response.sendRedirect("/manager/manageStaff");
